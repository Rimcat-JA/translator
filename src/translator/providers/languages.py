...
# Bundled capability subset, checked against the provider documentation on this
# date; it is not a claim that every provider language has been device-tested.
# https://docs.gladia.io/chapters/language/supported-languages
# https://developers.deepl.com/docs/getting-started/supported-languages
CAPABILITIES_CHECKED_AT = "2026-09-10"


def stt_language(code: str) -> str:
    if not get_language(code):
        raise ValueError("Unsupported source language")
    # Gladia uses Norwegian 'no'; the saved application/DeepL code is 'nb'.
    return "no" if code == "nb" else code


def capabilities() -> dict:
    return {
        "checked_at": CAPABILITIES_CHECKED_AT,
        "source": [{**item, "stt_code": stt_language(item["code"])} for item in SUPPORTED_LANGUAGES],
        "target": [dict(item) for item in SUPPORTED_LANGUAGES],
    }
