...
The preferred agent interface is the named JSON CLI in [agent-interface.json](agent-interface.json). See [agent operations](../docs/agents/operations.md). It discovers the local instance and exchanges its bootstrap internally; credentials are never returned by status commands. POST `/api/local/logout` requires the local cookie and CSRF, revokes only that cookie and returns `{status:"logged_out"}`. One-shot agent clients call it on close.

For bounded headless PC audio, an agent opens `/ws/local/events?audio_owner=OWNER_ID` with a 32-character lowercase hexadecimal ID. Add the same `owner_id` to system-audio mutations. The server binds it to the authenticated local principal. Starting requires that listener and an idle capture; another owner gets `AUDIO_BUSY`. An owner-scoped stop or listener closure releases only its own capture. Ordinary browser listener closure releases browser-owned capture only. Explicit session/application stop still ends all owned resources.
